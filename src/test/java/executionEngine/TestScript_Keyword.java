...
//	
	public void excute_TestCase() throws Exception {
		...
		// Không lấy hàng tiêu đề đầu tiên, đếm lần lượt
...
//				System.out.println(locatorValue);
	...
	static void execute_Actions() throws Exception {
		//sao cái này phải để vào trong vậy HOàng
		//khi nào chạy testcase mới tạo cái biến này oh , thế code dưới của t bị lỗi r 
		int casePass = 0;
		int caseFail = 0;
		int caseSkip = 0;
		boolean result;
...
//			break;
		...
		case "verifyLabel":
			...
			break;
			//HOàng chú thích code cho tớ với, để tớ hiểu code 
//		case "captrueScreen":
//			ActionKeywords.captureScreenshot("Login");
//			break;
		case "verifyHTML":
			resulthtml = ActionKeywords.verifyHTML5ValidValueField(locatorType, locatorValue, testData);
			if (resulthtml == true) {
				casePass++;

				System.out.println(ActionKeywords.verifyHTML5ValidValueField(locatorType, locatorValue, testData));
			} else {
				caseFail++;
				System.out.println(ActionKeywords.verifyHTML5ValidValueField(locatorType, locatorValue, testData));
				CaptureUtils.takeScreenshotCapture("Login");
...
//			ExtentTestManager.logMessage(Status.INFO,"Keyword Not Found :" + sActionKeyword);
		...
		System.out.println("==========================================================");
		SendMail.sendfile(casePass + caseFail + caseSkip,casePass, caseFail, caseSkip);
	}	
